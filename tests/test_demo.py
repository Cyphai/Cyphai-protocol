...
def test_demo(web3, chain):
    ...
    cyphai, deploy_tx = chain.provider.get_or_deploy_contract('Cyphai_demo')
    ...
    for i in range(len(training_partition)):
        ...
        iter_tx = cyphai.transact().init3(train_data[start:end], scd.train_nonce[i])
    ...
    for i in range(len(testing_partition)):
        ...
        iter_tx = cyphai.transact().reveal_test_data(test_data[start:end], scd.test_nonce[i])
        iter_receipt = wait_for_transaction_receipt(web3, iter_tx, timeout=timeout)
        total_gas_used += iter_receipt["gasUsed"]
